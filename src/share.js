...
export async function generateShareImage(primary, userLevels, dimOrder, dimDefs, mode) {
  ...
  for (const dim of dimOrder) {
    ...
    ctx.fillStyle = val === 3 ? '#d32f2f' : val === 2 ? '#ffca28' : '#8b9c94'
    ctx.fill()
  ...
  ctx.fillStyle = '#8b9c94'
  ctx.fillText('POKERTI 德扑人格测试 · 仅供娱乐', W / 2, H - cardY - 24)

  const link = document.createElement('a')
  link.download = `POKERTI-${primary.code}.png`
...
function drawShareRadar(ctx, cx, cy, maxR, userLevels, dimOrder, dimDefs) {
  ...
  for (let lv = 3; lv >= 1; lv--) {
    ...
    ctx.fillStyle = lv === 3 ? 'rgba(211,47,47,0.08)' : lv === 2 ? 'rgba(211,47,47,0.05)' : 'rgba(211,47,47,0.02)'
    ctx.fill()
